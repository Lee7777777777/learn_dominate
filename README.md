...
顶部「＋ 新建地图」通过设置卡片填写名称、选择背景，创建空白地图。通过「学习空间」下拉框切换地图，通过「地图设置」重命名和更换背景。切换前会提示保存未完成的笔记；同名地图会被阻止。
...
2. 选择模块，在右侧快速修改并点击「保存」；点击「完整编辑 ↗」或双击地图节点，可以在大编辑器里补充内容。编辑器支持 Ctrl+Enter / Ctrl+S 保存。缺少名称、重复关系等错误会在当前编辑器提示，保留已经输入的内容。
